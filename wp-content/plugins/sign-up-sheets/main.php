...
if (!class_exists('\FDSUS\Main')):
    ...
    class Main {
        ...
        function add_css_and_js_to_frontend()
        {
            ...
            wp_localize_script(
                Id::PREFIX . '-js',
                Id::PREFIX . '_validate_email',
                array('disable' => (!Settings::isEmailValidationEnabled()) ? 'true' : '')
            );
         ...
         *
         * @todo Update to use Notice class
         */
        public function dupPluginVersionCheck()
        {
            if ((Id::isPro() && is_plugin_active(Id::FREE_PLUGIN_BASENAME))
                ...
                if ($plugins[Id::FREE_PLUGIN_BASENAME]['Version']
                    ...
                    add_action('admin_notices', array($this, 'dupPluginVersionAdminNotice'));
                }
            }
        }
         ...
         */
        function dupPluginVersionAdminNotice()
        {
            if (!function_exists('get_plugins')) {
                require_once(ABSPATH . 'wp-admin/includes/plugin.php');
            }
            ...
            ?>
            <div id="<?php echo Id::PREFIX; ?>'-dup-plugin" class="error">
                <p><?php echo sprintf(
                    /* translators: %1$s is replaced with the pro plugin version number, %2$s is replaced with the free plugin version number */
                    esc_html__('The Sign-up Sheets Pro plugin version (%1$s) does not match the version number of the main Sign-up Sheets plugin (%2$s).  Please update so both version numbers match to prevent possible conflicts.'),

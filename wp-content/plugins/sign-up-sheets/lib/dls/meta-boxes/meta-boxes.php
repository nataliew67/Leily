 ...
 * @version 1.0.0.4
 * @author Liventus
 * @copyright 2013-2020 Liventus, Inc. <https://www.liventus.com/>
...
class MetaBoxes
{
    ...
    public function saveDetails($post_id, $post)
    {
        ...
        foreach ($this->meta_box['fields'] as $field) {
             ...
             */
            $check = apply_filters($this->prefix . "_update_post_metadata", null, $post->ID, $meta_key, $meta_value);
            if (null !== $check) {
                continue;
            }
    ...
    private function _displayInput($field, $value, $repeaterKey = null, $repeaterCount = 0)
    {
        ...
        switch ($field['type']) {
            ...
            case 'checkboxes':
                foreach ($field['options'] as $opt_key => $opt_val) {
                    echo sprintf(
                        '<input type="checkbox" name="%1$s[]" value="%2$s" class="dlsmb-field-element" id="%4$s" %5$s> <label for="%4$s">%3$s</label><br />',
                        $field_name,
                        $opt_key,
                        $opt_val,
                        sanitize_key($id . '-' . $opt_key),
                        in_array($opt_key, (array)$value) ? ' checked="checked"' : null
                    );
                }
                break;
            ...
            case 'multiselect':
                ...
                foreach ($field['options'] as $opt_key => $opt_val) {
                    $checked = (in_array($opt_key, (array)$value)) ? ' selected="selected"' : null;
                    echo '<option value="' . $opt_key . '" ' . $checked . '>' . $opt_val . '</option>';
                }
                echo '</select>';
                break;

...
class Base
{
    ...
    protected function cleanArray($input = array(), $prefix = false)
    {
        ...
        foreach ($input AS $k => $v) {
            if ($prefix === false || (substr($k, 0, strlen($prefix)) == $prefix)) {
                $cleanedFields[str_replace($prefix, '', $k)] = ($prefix == 'signup_' && !is_array($v))
    ...
    public function getExtraTaskDataBySheet($sheetId)
    {
        $sql = $this->wpdb->prepare("
            SELECT MIN(dates.meta_value) AS minDate, MAX(dates.meta_value) AS maxDate, COUNT(dates.meta_id) AS count
            FROM {$this->wpdb->posts} tasks
            LEFT JOIN {$this->wpdb->postmeta} dates ON tasks.ID = dates.post_id AND dates.meta_key = '" . Id::PREFIX . "_date'
            INNER JOIN {$this->wpdb->postmeta} header ON header.post_id = tasks.ID AND header.meta_key = '" . Id::PREFIX . "_task_row_type' AND header.meta_value <> 'header'
    ...
    {
        return admin_url('edit.php?post_type=dlssus_sheet&page=dlssus-settings');
    }

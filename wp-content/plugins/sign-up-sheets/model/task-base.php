...
class TaskBase extends Base
{
    ...
    public function setFields(&$post)
    {
        ...
        $parentSheet = new SheetModel($post->post_parent);
        if (!$parentSheet) {
    ...
    public function getSignups()
    {
        ...
        $this->signups = $signupCollection->getByTask($this->ID);
    ...
    public function isEmailOnTask($email)
    {
        ...
        foreach ($signups as $signup) {
            if (empty($signup->{Id::PREFIX . '_email'})) {
                continue;
            }
            if ($signup->{Id::PREFIX . '_email'} == $email) {
                return true;
            }
        }
        reset($signups);

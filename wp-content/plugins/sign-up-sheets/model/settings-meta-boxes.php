    ...
    public function getData()
    {
            ...
            'options' => array(
                ...
                array(
                    ...
                    'options' => array(
                        ...
                        array('label' => 'Options', 'name' => 'options', 'type' => 'textarea', 'note' => '*'),
        ...
        $options['form'] = array(
            ...
            'options' => array(
                ...
                array(
                    ...
                    'type'    => 'repeater',
                    'note'    => 'To add more fields, save this page and a new blank row will appear.<br />* Options are for checkbox, radio and dropdown fields.  Put multiple values on new lines.', // TODO translate
                    'options' => array(
                        array('label' => 'Name', 'name' => 'name', 'type' => 'text'),
                        array('label' => 'Slug', 'name' => 'slug', 'type' => 'text'),
                        array('label' => 'Type', 'name' => 'type', 'type' => 'dropdown', 'options' => $fieldTypesSignup),
                        array('label' => 'Options', 'name' => 'options', 'type' => 'textarea', '*'),
                        array('label' => 'Sheets', 'name' => 'sheet_ids', 'type' => 'multiselect', 'options' => $sheetSelection),
        ...
        $options['spam'] = array(
            ...
            'options' => array(
                ...
                array('reCAPTCHA Version', 'dls_sus_recaptcha_version', 'dropdown', '', array('v2-checkbox' => 'v2 Checkbox', 'v2-invisible' => 'v2 Invisible')),
            )
        );
        ...
        $reminderCheckUtcTimestamp = wp_next_scheduled('dls_sus_send_reminders');
        $displayReminderCheckTime = esc_html__('Not Scheduled', 'fdsus');
        if (!empty($reminderCheckUtcTimestamp)) {
            $timezoneString = get_option('timezone_string');
            if (empty($timezoneString)) {
                $reminderCheckUtcTimestamp = $reminderCheckUtcTimestamp + (get_option('gmt_offset', 0) * 60 * 60);
            }
            $reminderCheckDate = new DateTime(date('Y-m-d H:i:s', $reminderCheckUtcTimestamp), new DateTimeZone('UTC'));
            $reminderCheckDate->setTimezone(new DateTimeZone(wp_timezone_string()));
            $displayReminderCheckTime = $reminderCheckDate->format('M j, Y g:i a');
        }
        ...
        $options['reminder_email'] = array(
            ...
            'options' => array(
                array(
                    ...
                    'type'  => 'checkbox',
                    'note'  =>
                        /* translators: %s is replaced with the timestamp of the next reminder cron scheduled */
                        sprintf(esc_html__('Next scheduled reminder check: %s', 'fdsus'), $displayReminderCheckTime)
                        . '<ul>'
                            . '<li>'
                            ...
                            . '</li><li>'
                                . esc_html__('If you just enabled/disabled this, you may need to refresh this page to see the updated "Next scheduled reminder"', 'fdsus')
                            . '</li>'
                        . '</ul>',
                    'pro' => true
                ),
            ...
            if (!empty($option['options']) && is_array($option['options'])) {
                foreach ($option['options'] as $subKey => $subOption) {
                    if (!empty($subOption['pro'])) {
                        if (!Id::isPro()) {
                            $options[$key]['options'][$subKey]['label'] = '<span class="dls-sus-pro" title="Pro Feature">Pro</span> ' . $options[$key]['options'][$subKey]['label'];

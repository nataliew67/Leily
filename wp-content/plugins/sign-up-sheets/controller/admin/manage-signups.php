...
class ManageSignups
{
    ...
    public function page()
    {
        ...
        $signup_id = null;
        if (isset($_GET['signup_id']) && !is_numeric($_GET['signup_id'])) {
            wp_die(esc_html__('Invalid signup ID.', 'fdsus'));
        } elseif (isset($_GET['signup_id'])) {
            $signup_id = (int)$_GET['signup_id'];
        }

        $sheet = new SheetModel($sheet_id);
        if (!is_object($sheet)) {

...
class TaskTable extends Base
{
            ...
            if (!$this->config['isFullCompact']) {
                if (!empty($signups) && is_array($signups)) {
                    foreach ($signups AS $signup) {
                        ...
                        $cellValue = sprintf(
                            '<span class="dls-sus-spot-num">%s</span> <em>%s</em>',
                        ...
                        if ($this->config['displayAll'] && is_admin()) {
                            $clear_url = wp_nonce_url(
                                '?post_type=' . SheetModel::POST_TYPE . '&amp;action=clear&amp;page=dlssus-manage&amp;sheet_id=' . $_GET['sheet_id'] . '&amp;clear=' . $signup->ID,
                            ...
                            $cellValue = sprintf(
                                ...
                                <span class="delete">
                                    <label>
                                        <span class="sr-only">' . sprintf(__('Select spot #%s to clear', 'fdsus'), (int)$this->config['spotIndex']) . '</span>
    ...
    public function output()
    {
        ...
        $formUrl = add_query_arg(
            ...
            remove_query_arg(array('action', 'status', 'tasks', 'signups'))
        );

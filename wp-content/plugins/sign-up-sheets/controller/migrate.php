...
class Migrate extends MigrateParent
{
    ...
    /**
     * Get overview of migration
     *
     * @param bool $full get full details instead of just summary
     */
    public function getOverview($full = false)
    {
        global $wpdb;

        //$wpdb->query("DROP TEMPORARY TABLE newitem;");

        $wpdb->query($wpdb->prepare("
            CREATE TEMPORARY TABLE IF NOT EXISTS newitem
                SELECT newitem.ID, newitem.post_title, oldid.meta_value as oldid, newitem.post_type FROM {$wpdb->posts} newitem
                LEFT JOIN {$wpdb->postmeta} oldid ON oldid.post_id = newitem.ID AND oldid.meta_key = 'dlssus_id_v2_0'
                WHERE newitem.post_type IN (%s, %s, %s);
            ",
            SheetModel::POST_TYPE,
            TaskModel::POST_TYPE,
            SignupModel::POST_TYPE
        ));

        $fullSql = "
            SELECT
                olditem.post_type
                , olditem.id AS old_id
                , olditem.title AS old_description
                , GROUP_CONCAT(newitem.ID SEPARATOR ', ') AS new_id
                , COUNT(newitem.ID) as new_id_count
                , IF(COUNT(newitem.ID) = 1, 1, NULL) AS successful
                , IF(COUNT(newitem.ID) = 0, 1, NULL) AS not_migrated
                , IF(COUNT(newitem.ID) > 1, 1, NULL) AS duplicate
            FROM (
                -- Get non-orphaned old items
                SELECT id, title, 'sheet' AS post_type FROM {$wpdb->prefix}dls_sus_sheets sheet -- 58
                UNION SELECT id, title, 'task' AS post_type
                    FROM (SELECT * FROM {$wpdb->prefix}dls_sus_tasks WHERE sheet_id IN (SELECT ID FROM {$wpdb->prefix}dls_sus_sheets)) task -- 203
                UNION SELECT id, email AS title, 'signup' AS post_type
                    FROM (SELECT * FROM {$wpdb->prefix}dls_sus_signups WHERE task_id IN (SELECT ID FROM {$wpdb->prefix}dls_sus_tasks)) signup -- 480
            ) olditem
            LEFT JOIN newitem ON newitem.oldid = olditem.id AND newitem.post_type = CONCAT('dlssus_', olditem.post_type)
            GROUP BY olditem.id, olditem.title, olditem.post_type
            ORDER BY olditem.post_type, olditem.id";

        $summarySql = "
            SELECT
                post_type
                , SUM(successful) AS successful
                , SUM(not_migrated) AS not_migrated
                , SUM(duplicate) as duplicate
            FROM ($fullSql) AS migrate_status
            GROUP BY post_type;";

        $results = $wpdb->get_results($full ? $fullSql : $summarySql);
        return $results;
    }

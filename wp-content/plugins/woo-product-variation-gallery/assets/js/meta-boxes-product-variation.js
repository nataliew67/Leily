...
/*
    The file is 100% same. Just copy the latest file. and Pest below. Don't Hesited to do that
    https://plugins.trac.wordpress.org/browser/woocommerce/tags/7.7.0/assets/js/admin/meta-boxes-product-variation.js?desc=1
*/
jQuery(function ($) {
  'use strict';
  /**
  ...
  var wc_meta_boxes_product_variations_actions = {
    ...
    create_variations: function create_variations(event) {
     ...
     */
    maybe_enable_button_to_add_price_to_variations: function maybe_enable_button_to_add_price_to_variations() {
    ...
    variations_loaded: function variations_loaded(event, needsUpdate) {
      ...
      if (!needsUpdate) {
        ...
        $('input.variable_is_downloadable, input.variable_is_virtual, input.variable_manage_stock', wrapper).trigger('change'); // Open sale schedule fields when have some sale price date
     ...
     */
    set_variations_price: function set_variations_price() {
      var variation_price = $('.wc_input_variations_price').val();
      var product_type = $('select#product-type').val();
      var input_type = 'variable-subscription' === product_type ? 'variable_subscription_sign_up_fee' : 'variable_regular_price';
      var input = $(".wc_input_price[name^=".concat(input_type, "]")); // We don't want to override prices already set
     ...
     */
    open_modal_to_set_variations_price: function open_modal_to_set_variations_price() {
  ...
  var wc_meta_boxes_product_variations_ajax = {
     ...
     */
    input_changed: function input_changed(event) {
      $(this).closest('.woocommerce_variation').addClass('variation-needs-update');
      $('button.cancel-variation-changes, button.save-variation-changes').prop('disabled', false); // Do not trigger 'woocommerce_variations_input_changed' for 'input' events for backwards compat.
     ...
     */
    show_hide_variation_empty_state: function show_hide_variation_empty_state() {
      var wrapper = $('#variable_product_options').find('.woocommerce_variations');
     ...
     * Generate variations
     */
    generate_variations: function generate_variations() {
     ...
     * Add variation
     */
    add_variation_manually: function add_variation_manually() {
